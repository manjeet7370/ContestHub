      ...
      <div className="lg:col-span-5 bg-slate-900 rounded-xl flex flex-col min-h-0 overflow-hidden border border-slate-800">
        ...
        <div className="px-6 py-4 border-b border-slate-800 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-3 min-w-0">
            <span className="text-slate-500 text-xs font-mono uppercase tracking-widest shrink-0">Problem</span>
            <span className="text-slate-700 text-xs">·</span>
            <h1 className="text-white text-sm font-semibold truncate">{problem.title}</h1>
          </div>
          <span
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-7">
          ...
          {/* Statement */}
          <p className="text-slate-300 text-sm leading-7">{problem.statement}</p>

          {/* Examples */}
          {problem.testCases?.length > 0 && (
            <div className="space-y-4">
              <h2 className="text-slate-400 text-xs font-semibold uppercase tracking-widest">Examples</h2>
              ...
              {problem.testCases.map((testCase, index) => (
                ...
                >
                  <div className="px-4 py-2 border-b border-slate-700 bg-slate-800">
                    <span className="text-slate-400 text-xs font-medium">Example {index + 1}</span>
                  </div>

                  <div className="p-4 space-y-3">
                    <div>
                      <span className="text-slate-500 text-xs font-medium uppercase tracking-wide block mb-1">Input</span>
                    ...
                    <div>
                      <span className="text-slate-500 text-xs font-medium uppercase tracking-wide block mb-1">Output</span>
                    ...
                    {testCase.explanation && (
                      <div>
                        <span className="text-slate-500 text-xs font-medium uppercase tracking-wide block mb-1">Explanation</span>
                        <p className="text-slate-400 text-xs leading-5">{testCase.explanation}</p>
                      </div>
                    )}
                  </div>
                </div>
      ...
      <div className="lg:col-span-7 bg-slate-900 rounded-xl flex flex-col min-h-0 overflow-hidden border border-slate-800">
        ...
        <div className="shrink-0 bg-slate-800 border-b border-slate-700 px-4 py-2.5 flex items-center justify-between gap-3">
          <div className="flex items-center gap-2">
            <label htmlFor="language-select" className="text-slate-400 text-xs font-medium">
              Language
            </label>
            <select
              id="language-select"
